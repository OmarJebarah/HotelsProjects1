...
public class MovinpickController implements Initializable {
    @FXML
    ...
    public static final ObservableList<Rooms> listM = FXCollections.observableArrayList();
    ...
    public void RoyalRoomsList() throws Exception {
        String sql = "SELECT * FROM rooms WHERE hotel_name='Movenpick'";
        PreparedStatement ps = DbConnection.ConnectDB().prepareStatement(sql);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            listM.add(new Rooms(Integer.parseInt(rs.getString("room_id")), rs.getInt("room_number"), rs.getInt("cost"), rs.getString("status")));
        }
    }
}
